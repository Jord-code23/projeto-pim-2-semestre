...
const API_URL = "https://localhost:7001/api"; // Ajuste a porta conforme sua API
...
// Nova função para Registrar
async function registerUser(name, email, phone, password) {
    try {
        const response = await fetch(`${API_URL}/auth/register`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ nome: name, email: email, telefone: phone, senha: password })
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.message || 'Erro no cadastro');
        alert('Cadastro realizado! Faça o login.');
        showLoginForm(); // Função para alternar o modal para login
    } catch (error) {
        alert(error.message);
    }
}
...
async function loginUser(email, password) {
    try {
        ...
        closeLoginModal(); // Fechar modal de login
        updateUIForLoggedInUser(); // Atualizar botões (ex: mostrar "Bem-vindo, Nome")
        
        // Se o usuário estava tentando agendar, continua o processo
        if (pendingBooking) {
            sendBookingToAPI();
        }
    } catch (error) {
...
function confirmBooking() {
    ...
    const phone = document.getElementById('clientPhone')?.value; // Adicionado para evitar erros se a variável não existia
    ...
    // Fecha o modal e exibe o aviso de sucesso
    closeBookingModal();
    showConfirmationToast();

    // Reseta o estado do agendamento para o valor inicial
    bookingState = {
        currentStep: 1,
    ...
    if (!isUserLoggedIn()) {
        // Salva os dados do agendamento pendente
        pendingBooking = { clientName: name, clientPhone: phone }; 
        openLoginModal(); // Abre o modal de login que você criará no HTML
        return;
    }
...
async function sendBookingToAPI() {
    ...
    const dateTime = `${dateStr}T${bookingState.selectedTime}:00`;

    const payload = {
        servicoId: bookingState.selectedService,
        barbeiroId: bookingState.selectedBarber,
        dataHora: dateTime,
        // ClienteId é pego pelo token no backend
    };
    ...
    try {
        ...
        if (response.ok) {
            closeBookingModal();
            showConfirmationToast();
            bookingState = { ...initialState }; // Resetar estado
        } else {
